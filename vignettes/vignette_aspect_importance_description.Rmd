...
Aspect importance method takes on the challenge of interpreting model built on highly dimensional data. There exists number of methods for local explanation of black-box models, like Break Down, Shap or LIME. However, the problem arise when the explanatory variables are highly correlated.

Aspect importance goal is to increase the interpretability of the black box model by providing instance-level explainer for the groups of explanatory variables. It allows us to group predictors into entities called aspects. Afterwards, it can calculate the contribution of those aspects to the prediction.
...
When the dataset contains only numerical varaibles, we can use `triplot()` to verify the values of aspects importance at different levels of variables grouping. 

`Triplot` allows us to check importance of ....

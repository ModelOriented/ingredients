...
cp_rf <- ceteris_paribus(explain_titanic_rf, 
                         passanger)
plot(cp_rf, only_numerical = TRUE, variables = perturbed_variable_continuous)
...
### Partial and Accumulated Dependency Profiles

```{r}
pdp <- aggregate_profiles(cp_rf, type = "partial", only_numerical = TRUE)
plot(pdp, variables = "fare")
describe(pdp, variables = "fare")
```

```{r}
pdp <- aggregate_profiles(cp_rf, type = "accumulated", only_numerical = FALSE)

...
#' @param ... other explanations that shall be calculated together
...
aggregated_profiles_accumulated <- function(all_profiles, groups = NULL, span = 0.25) {
  ...
  chunks <- lapply(split_profiles, function(split_profile) {
    ...
    #  diffs
    per_points <- split(split_profile, split_profile$`_ids_`, drop = TRUE)
    ...
    # weighed means
    per_points <- split(split_profile, split_profile[, c("_x_", groups)], drop = TRUE)
    chunks <- lapply(per_points, function(per_point) {
    ...
    if (is.null(groups)) {
      ...
      # cumsum per group
      per_points <- split(split_profile, split_profile[, groups], drop = TRUE)
      chunks <- lapply(per_points, function(per_point) {
